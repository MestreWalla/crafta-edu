$(document).ready(function () {
  ...
  $(".zona-soltar").click(function () {
    if ($(this).hasClass("preenchido")) {
      $(this)
        .removeClass("preenchido")
        .text(
          $(this).attr("id") === "numero1"
            ? "Solte o Número 1 aqui"
            : $(this).attr("id") === "operador"
            ? "Solte o Operador aqui"
...
function calcular() {
  ...
  if (!numero1 || !operador || !numero2) {
    resultadoContainer.html(
      '<span class="resultado" style="background-color: #7f8c8d" >?</span>'
    );
    return;
  }
  ...
  switch (operador) {
    ...
    case "/":
      ...
      if (num2 === 0) {
        resultadoContainer.html(
          '<span class="resultado" style="background-color: #7f8c8d">Divisão por zero!</span>'
        );
        return;
      }
      resultado = num1 / num2;
    ...
    default:
      resultadoContainer.html(
        '<span class="resultado" style="background-color: #7f8c8d">Operador inválido</span>'
      );
      return;
  }
  ...
  if (isValid) {
    ...
    resultadoContainer.html(
      `<span class="resultado" style="background-color: ${bgColor}">${resultado}</span>`
    );
  }
}
