...
class CarregadorDoJogo {
  constructor() {
    ...
    for (const chave in this.elementos) {
      if (!this.elementos[chave] && chave !== "primeiraOpcaoJogo") {
        console.error(
          `[CarregadorDoJogo] Elemento essencial não encontrado: ${chave}. A aplicação pode não funcionar corretamente.`
        );
      }
    }
  ...
  inicializar() {
    ...
    this.preCarregarAssets()
      ...
      .catch((erro) => {
        console.error(
          "[CarregadorDoJogo] Erro no pré-carregamento de assets:",
          erro
        );
        this.iniciarSequencia();
      });
  ...
  registrarEventos() {
    [this.elementos.videoCarregamento, this.elementos.videoIntro].forEach(
      (video) => {
        if (!video) return;

        video.addEventListener("error", (e) => {
          console.warn(
            `[CarregadorDoJogo] Erro ao carregar vídeo: ${video.id}`,
            e.target.error
          );

          if (
            video.id === "video-loading" &&
            this.estadoAtual === "carregando"
          ) {
        ...
        video.addEventListener("stalled", (e) => {
          console.warn(
            `[CarregadorDoJogo] Carregamento do vídeo ${video.id} interrompido (stalled).`
          );
        });

        video.addEventListener("loadeddata", () => {
          console.log(
            `[CarregadorDoJogo] Dados do vídeo ${video.id} carregados.`
          );
        });
      }
    );
    ...
    document.addEventListener("keydown", (e) => {
      if (["Escape", " ", "Enter"].includes(e.key)) {
        console.log(
          "[CarregadorDoJogo] Tecla pressionada. Pulando para tela de escolha."
        );
        this.introPulado = true;
        this.mostrarTelaEscolha(true);
      }
  ...
  async iniciarSequencia() {
    try {
      this.estadoAtual = "carregando";
      await this.reproduzirFaseVideo(
        this.elementos.videoCarregamento,
        "video-loading"
      );
      ...
      console.error(
        "[CarregadorDoJogo] Erro na sequência principal:",
        erro.message,
        erro.details || ""
      );
      this.mostrarTelaEscolha(true);
  ...
  async reproduzirFaseVideo(video, nome, videoAnterior = null) {
    if (!video) {
      console.warn(
        `[CarregadorDoJogo] Elemento de vídeo ${nome} não encontrado. Pulando esta fase.`
      );
    ...
    return new Promise(async (resolve, reject) => {
      ...
      const aoErro = (err) => {
        ...
        video.removeEventListener("error", aoErro);
        console.warn(
          `[CarregadorDoJogo] Erro ao reproduzir vídeo ${nome}:`,
          err
        );
      ...
      try {
        await video.play();
        console.log(
          `[CarregadorDoJogo] Iniciando reprodução do vídeo ${nome}.`
        );
      } catch (erroReproducao) {
        console.warn(
          `[CarregadorDoJogo] Autoplay bloqueado ou falhou:`,
          erroReproducao
        );
        clearTimeout(timeout);
        video.removeEventListener("ended", aoFinalizar);
  ...
  pularParaIntroOuEscolha() {
    console.warn(
      "[CarregadorDoJogo] Vídeo de carregamento falhou. Pulando para intro ou escolha."
    );
    this.ocultarElemento(
      this.elementos.videoCarregamento,
      NOMES_DAS_CLASSES.VIDEO_OCULTO
    );
  ...
  mostrarTelaEscolha(imediato = false) {
    ...
    if (this.elementos.telaEscolha) {
      this.elementos.telaEscolha.classList.remove(
        NOMES_DAS_CLASSES.TELA_ESCOLHA_OCULTA
      );
      this.elementos.telaEscolha.classList.add(
        NOMES_DAS_CLASSES.TELA_ESCOLHA_VISIVEL
      );
      ...
      console.error(
        "[CarregadorDoJogo] Elemento 'telaEscolha' não encontrado."
      );
    }
  }
  ...
  mostrarBotaoPular() {
    if (this.elementos.botaoPularIntro) {
      this.elementos.botaoPularIntro.classList.remove(
        NOMES_DAS_CLASSES.BOTAO_PULAR_VISIVEL.replace("--visible", "--hidden")
      );
      this.elementos.botaoPularIntro.classList.add(
        NOMES_DAS_CLASSES.BOTAO_PULAR_VISIVEL
      );
    }
  }
